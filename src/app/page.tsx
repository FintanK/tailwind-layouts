...
const categorizeLayouts = (names: string[]): { [key: string]: string[] } => {
  ...
  const categoryKeywords: { keyword: string[], category: string }[] = [
    ...
    { keyword: ['about us section with image', 'about us'], category: 'About Us Section' },
    ...
    { keyword: ['alert - action buttons', 'alert - simple accent border', 'alert - top banner fixed', 'alert - with description', 'notification - floating alerts', 'notification - simple banner', 'notification - toast style bottom right', 'alert - dismissible banner', 'status indicator with text', 'status badges', 'alert banner', 'alert modal', 'notification list', 'toast notification', 'snackbar'], category: 'Alert/Notification' },
